...
class AssistantFlow(Workflow):
    ...
    async def reply(self, ev: SanityCheckEvent, ctx: Context) -> StopEvent:
        ...
        query_clean = "разбился на авто"
        qa = [
            (
                "я сменил автомобить, на учет еще не поставил, могу ли я заправляться по топливной карте?",
                "для внесения данных по личному автомобилю обратитесь, пожалуйста, к своему руководителю для создания заявки по теме "
                "изменение режима характера работы"
                ", подтема "
                "установка рхр и топливной карты"
                ". в комментариях опишите ситуацию и приложите птс, стс, страховой полис и водительское удостоверение.",
            ),
            (
